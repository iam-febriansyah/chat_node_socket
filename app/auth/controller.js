...
module.exports = {
    signUp: async (req, res) => {
        ...
        try {
            ...
            if (cek) {  throw "Email already exist"; }
            ...
            var email = await help.sendMail(
                email.trim(),
                "Activate Account",
                "Activate your account with click link below<br>" + activateUrl,
                null
            );
    ...
    changeForgotPassword: async (req, res) => {
        ...
        try {
            ...
            var email = await help.sendMail(
                ...
                "Congratulation to succes change your password!",
                null
            );
            res.send({
                status: true,
    ...
    changePassword: async (req, res) => {
        ...
        try {
            ...
            if (!update) { throw "Failed change password"; }
    ...
    editAccount: async (req, res) => {
        ...
        try {
            ...
            var cek = await db.users.findOne({ where: { user_id: req.session.user_id  } });
            if (!cek) { throw "User doesn't exist"; }
            ...
            if(email != cek.email){
                ...
                if (!update) { throw "Failed edit account email"; }
            }

            const updateD = await db.user_details.update(
                { fullname: fullname },
                { where: { user_id: req.session.user_id } }
            );
            if (!updateD) { throw "Failed edit account name"; }
