...
async function read(from_user_id, to_user_id, req){
    ...
    await db.chats.update({ read_at : read_at }, { 
        where : { 
            read_at : { [Op.eq] : null },
    ...
    listUser: async (req, res) => {
        ...
        try {
            ...
            var users = await db.users.findAll({ 
                where : { activate : { [Op.eq] : null }, user_id : { [Op.not]: user_id }  },
                offset :((page-1)*limit),
                limit : limit,
                subQuery : false
            })
    ...
    listChatUser: async (req, res) => {
        ...
        try {
            ...
            var query = {
                attributes: [
                  '*',
                  [db.Sequelize.literal(`CASE WHEN from_user_id = '${from_user_id}' THEN '1' ELSE '0' END`), 'is_me']
                ],
                where: {
            ...
            var chats = await db.chats.findAll(query).catch((err)=>{ console.log(err); return false; });;
